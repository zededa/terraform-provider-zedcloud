...
// isAutoGeneratedVF reports whether an io_member is a virtual function that
// zedcloud auto-creates for an SR-IOV physical function (IO_TYPE_ETH_PF) based
// on its vfs.count. Such members (ztype IO_TYPE_ETH_VF with a parentassigngrp)
// are never present in the terraform config, so they must not be treated as
// drift when comparing config against the backend state.
func isAutoGeneratedVF(m *models.IoMember) bool {
	return m != nil && m.Ztype != nil &&
		*m.Ztype == models.IoTypeIOTYPEETHVF && m.Parentassigngrp != ""
}

// filterAutoGeneratedVFs returns only the io_members managed by terraform
// config, dropping the VF members that zedcloud generates on its own.
func filterAutoGeneratedVFs(members []*models.IoMember) []*models.IoMember {
	filtered := make([]*models.IoMember, 0, len(members))
	for _, m := range members {
		if isAutoGeneratedVF(m) {
			continue
		}
...
// vfsCount returns the configured number of SR-IOV virtual functions for an
// io_member, or 0 when none are set.
func vfsCount(m *models.IoMember) int64 {
	if m == nil || m.Vfs == nil {
		return 0
	}
	return m.Vfs.Count
