...
func diffSuppressIoMemberListOrder(mapKey string) schema.SchemaDiffSuppressFunc {
	return func(key, oldValue, newValue string, d *schema.ResourceData) bool {
		...
		// zedcloud auto-creates VF io_members for SR-IOV physical functions,
		// so the backend state can contain more members than the config. Drop
		// those before comparing, otherwise the length mismatch is reported as
		// spurious drift (CI-865).
		return CompareIoMemberLists(filterAutoGeneratedVFs(oldMapList), filterAutoGeneratedVFs(newMapList))
	}
}
