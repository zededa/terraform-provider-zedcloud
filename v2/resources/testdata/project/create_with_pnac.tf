...
resource "zedcloud_project" "test_tf_provider_pnac" {
  ...
  title = "Test PNAC Project__SUFFIX__"
  type  = "TAG_TYPE_PROJECT"

  attestation_policy {
    title = "test_tf_provider_pnac_attest"
  ...
  port_based_network_access_control_policy {
    title = "Test PNAC Policy"
    type  = "POLICY_TYPE_PORT_BASED_NETWORK_ACCESS_CONTROL"

    port_based_network_access_control_policy {
      enable_port_based_network_access_control = true
      eap_method                               = "EAP_METHOD_TLS"
      eap_identity                             = "device-identity"
      certificate_enrollment_id                = zedcloud_cep_profile.test_tf_provider_pnac_cep.id
    }
  }
}
