...
func (m *Node) validateModel(formats strfmt.Registry) error {
	if m.Model != nil {
		if err := m.Model.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("body" + "." + "model")
			} else if ce, ok := err.(*errors.CompositeError); ok {
				return ce.ValidateName("body" + "." + "model")
			}
			return err
		}
	}
...
// ContextValidate validate this edge node configuration update edge node base o s body based on the context it is used
