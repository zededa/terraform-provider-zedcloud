...
// FIXME: use error wrapping and return error instead of string
func doVerifyFlattenOutput(
	...
	for k, v := range output {
		...
		case schema.TypeList:
			...
			if !ok {
				errStr += fmt.Sprintf("Key: %s - Value not List. Value: %#v, type(v): %T", pkey(parentKeys, k), v, v)
				continue
			}
			parentKeyVal := pkey(parentKeys, k)
			for i, entry := range listVal {
				entryParentKey := fmt.Sprintf("%s[%d]", parentKeyVal, i)
				listEntryVal, ok := entry.(map[string]interface{})
				if ok {
					elem, ok := schemaEntry.Elem.(*schema.Resource)
					if ok {
						errStr += doVerifyFlattenOutput(elem.Schema, listEntryVal, checkAllKeysPresent, entryParentKey)
					}
				}
			}
		case schema.TypeMap:
			...
			if ok {
				...
				if !ok {
					errStr += fmt.Sprintf(
						"Key: %s - Value not map[string]interface{} as expected. Value: %#v, Type: %T",
						pkey(parentKeys, k),
						v,
						v)
					continue
				...
				if elem.Type == schema.TypeString {
					...
					if !ok {
						errStr += fmt.Sprintf(
							"Key: %s - Value not map[string]string as expected. Value: %#v, Type: %T",
							pkey(parentKeys, k),
							v,
							v)
						continue
