...
func ModuleTypeModel(d *schema.ResourceData) *models.ModuleType {
	moduleType := d.Get("module_type").(models.ModuleType)
	return &moduleType
}
...
// Iterate throught and update the ModuleType resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
