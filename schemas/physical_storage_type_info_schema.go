...
func PhysicalStorageTypeInfoModel(d *schema.ResourceData) *models.PhysicalStorageTypeInfo {
	physicalStorageTypeInfo := d.Get("physical_storage_type_info").(models.PhysicalStorageTypeInfo)
...
// Iterate throught and update the PhysicalStorageTypeInfo resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
