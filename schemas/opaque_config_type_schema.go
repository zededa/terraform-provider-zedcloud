...
func OpaqueConfigTypeModel(d *schema.ResourceData) *models.OpaqueConfigType {
	opaqueConfigType := d.Get("opaque_config_type").(models.OpaqueConfigType)
...
// Iterate throught and update the OpaqueConfigType resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
