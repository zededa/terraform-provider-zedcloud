...
func DeviceSWStatusModel(d *schema.ResourceData) *models.DeviceSWStatus {
	deviceSWStatus := d.Get("device_s_w_status").(models.DeviceSWStatus)
...
// Iterate throught and update the DeviceSWStatus resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
