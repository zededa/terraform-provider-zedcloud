...
func PolicyStatusModel(d *schema.ResourceData) *models.PolicyStatus {
	policyStatus := d.Get("policy_status").(models.PolicyStatus)
	return &policyStatus
}
...
// Iterate throught and update the PolicyStatus resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
