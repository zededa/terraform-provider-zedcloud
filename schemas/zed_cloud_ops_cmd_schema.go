...
func ZedCloudOpsCmdModel(d *schema.ResourceData) *models.ZedCloudOpsCmd {
	counter := int64(d.Get("counter").(int))
	opsTime := d.Get("ops_time").(interface{}) // interface{}
...
// Iterate throught and update the ZedCloudOpsCmd resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
