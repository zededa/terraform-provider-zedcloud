...
func DeviceDataSecurityAtRestStatusModel(d *schema.ResourceData) *models.DeviceDataSecurityAtRestStatus {
	deviceDataSecurityAtRestStatus := d.Get("device_data_security_at_rest_status").(models.DeviceDataSecurityAtRestStatus)
...
// Iterate throught and update the DeviceDataSecurityAtRestStatus resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
