...
func AttestPolicyTypeModel(d *schema.ResourceData) *models.AttestPolicyType {
	attestPolicyType := d.Get("attest_policy_type").(models.AttestPolicyType)
...
// Iterate throught and update the AttestPolicyType resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
