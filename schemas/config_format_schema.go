...
func ConfigFormatModel(d *schema.ResourceData) *models.ConfigFormat {
	configFormat := d.Get("config_format").(models.ConfigFormat)
	return &configFormat
}
...
// Iterate throught and update the ConfigFormat resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
