...
func NetworkProxyProtoModel(d *schema.ResourceData) *models.NetworkProxyProto {
	networkProxyProto := d.Get("network_proxy_proto").(models.NetworkProxyProto)
...
// Iterate throught and update the NetworkProxyProto resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
