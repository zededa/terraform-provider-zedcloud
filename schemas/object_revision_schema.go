...
func ObjectRevisionModel(d *schema.ResourceData) *models.ObjectRevision {
	createdAt := d.Get("created_at").(interface{}) // interface{}
	createdBy := d.Get("created_by").(string)
	curr := d.Get("curr").(string)
	prev := d.Get("prev").(string)
	updatedAt := d.Get("updated_at").(interface{}) // interface{}
...
// Iterate throught and update the ObjectRevision resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
