...
func CertificateModel(d *schema.ResourceData) *models.Certificate {
	basicContraintsValid := d.Get("basic_contraints_valid").(bool)
	...
	keyUsage := int32(d.Get("key_usage").(int))
	passPhrase := d.Get("pass_phrase").(string)
	publicKey := d.Get("public_key").(string)
	publicKeyAlgorithm := d.Get("public_key_algorithm").(string)
	pvtKey := d.Get("pvt_key").(string)
	reuseKey := d.Get("reuse_key").(bool)
	...
	validFrom := d.Get("valid_from").(strfmt.DateTime)
	validTill := d.Get("valid_till").(strfmt.DateTime)
...
// Iterate throught and update the Certificate resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
