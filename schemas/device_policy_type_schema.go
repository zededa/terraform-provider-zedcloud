...
func DevicePolicyTypeModel(d *schema.ResourceData) *models.DevicePolicyType {
	devicePolicyType := d.Get("device_policy_type").(models.DevicePolicyType)
...
// Iterate throught and update the DevicePolicyType resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
