...
func DeviceHWSecurityModuleStatusModel(d *schema.ResourceData) *models.DeviceHWSecurityModuleStatus {
	deviceHWSecurityModuleStatus := d.Get("device_h_w_security_module_status").(models.DeviceHWSecurityModuleStatus)
...
// Iterate throught and update the DeviceHWSecurityModuleStatus resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
