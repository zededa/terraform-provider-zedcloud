...
func AttestStateModel(d *schema.ResourceData) *models.AttestState {
	attestState := d.Get("attest_state").(models.AttestState)
	return &attestState
}
...
// Iterate throught and update the AttestState resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
