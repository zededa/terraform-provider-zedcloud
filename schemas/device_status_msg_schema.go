...
// Iterate throught and update the DeviceStatusMsg resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
func SetDeviceStatusMsgSubResourceData(m []*models.DeviceStatusMsg) (d []*map[string]interface{}) {
	for _, DeviceStatusMsgModel := range m {
		if DeviceStatusMsgModel != nil {
			...
			properties["boot_time"] = DeviceStatusMsgModel.BootTime
			...
			properties["last_reboot_time"] = DeviceStatusMsgModel.LastRebootTime
			properties["last_update"] = DeviceStatusMsgModel.LastUpdate
			...
			properties["up_time"] = DeviceStatusMsgModel.UpTime
