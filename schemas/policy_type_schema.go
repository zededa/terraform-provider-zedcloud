...
func PolicyTypeModel(d *schema.ResourceData) *models.PolicyType {
	policyType := d.Get("policy_type").(models.PolicyType)
	return &policyType
}
...
// Iterate throught and update the PolicyType resource data within a pagination response (typically defined in the items array field) retrieved from a READ operation for multiple LM resources
